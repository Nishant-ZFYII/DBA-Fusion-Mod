...
if __name__ == '__main__':
    ...
    args.ref_file = '/home/lunar/DBA-Fusion/datasets/tumvi/512_16_euroc/dataset-%s_512_16/dso/gt_imu.csv' % seq
    ...
    for iii in range(len(args.est_files)):
        ...
        print(f"[DEBUG] traj_ref: {len(traj_ref.timestamps)} poses")
        print(f"[DEBUG] traj_est: {len(traj_est.timestamps)} poses")
        print(f"[DEBUG] First timestamps:\n  GT: {traj_ref.timestamps[0] if len(traj_ref.timestamps) > 0 else 'EMPTY'}\n  EST: {traj_est.timestamps[0] if len(traj_est.timestamps) > 0 else 'EMPTY'}")

        traj_ref_sel, traj_est_sel = sync.associate_trajectories(
            traj_ref, traj_est, 0.01,0.0,
            first_name=ref_name, snd_name=est_name)
        print(f"[DEBUG] First 5 GT timestamps: {traj_ref.timestamps[:5]}")
        print(f"[DEBUG] First 5 EST timestamps: {traj_est.timestamps[:5]}")

        # use the starting part for scale estimation
        print("[DEBUG] Loaded reference trajectory length:", len(traj_ref_sel.timestamps))

        args.n_to_align = np.where((np.array(traj_ref_sel.timestamps)[1:]-np.array(traj_ref_sel.timestamps)[0:-1])>0.100)[0][0]-1
