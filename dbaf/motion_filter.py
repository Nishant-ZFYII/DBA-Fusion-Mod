...
class MotionFilter:
    ...
    def track(self, tstamp, image, depth=None, intrinsics=None):
        ...
        #for the first frrame we get the identity to tbe the squeezed data set for reference
        Id = lietorch.SE3.Identity(1,).data.squeeze()

        #this is done to make sure the input is in the right format for the CNN clusters
        ht = image.shape[-2] // 8
        wd = image.shape[-1] // 8

        # normalize images
        # add a empty batch dimension -> this is how CNN processess data
        
        inputs = image[None, :, [2,1,0]].to(self.device) / 255.0
        # subtract mean and std_deviation of the rgb values
        #This process, known as standardization, centers the data for each channel around zero and scales it, which helps the neural network learn more effectively.
